...
export default function App(){
    ...
    const createTodo = async () => {
        const body = {...newTodo}
        try {
            const response = await fetch('http://34.203.226.236:8000/todos/', {
                method: 'POST',
                headers: {
    ...
    const deleteTodo = async (id) => {
        try {
            await fetch(`http://34.203.226.236:8000/todos/${id}/`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    const moveToCompleted = async (id) => {
        try {
            ...
            const response = await fetch(`http://34.203.226.236:8000/todos/${id}/`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
